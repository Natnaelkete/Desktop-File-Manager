...
const SettingsPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "security" | "booster">("general");
    ...
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden h-full select-none">
      ...
      <div className="h-14 flex items-center justify-between px-6 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-10 transition-all">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wide">Settings</h2>
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-emerald-500/5 dark:bg-emerald-500/10 rounded-full border border-emerald-500/10">
          ...
          <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Active System</span>
        </div>
      </div>
          ...
          {tabs.map((tab) => (
            ...
            >
              <div className={clsx(
                ...
                activeTab === tab.id ? "bg-primary-500 text-white" : "bg-slate-100 dark:bg-slate-800"
              )}>
                <tab.icon size={16} />
              </div>
          ...
          <div className="max-w-3xl mx-auto p-8 space-y-8">
            {activeTab === "general" && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <section>
                  <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Environment</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Theme Switcher */}
                    <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200">Theme Mode</span>
                      ...
                      <div className="flex bg-slate-50 dark:bg-slate-950 p-1 rounded-lg">
                        <button 
                          ...
                          className={clsx(
                            ...
                            theme === "light" ? "bg-white text-primary-500 shadow-sm" : "text-slate-500"
                          )}
                        >
                          Light
                        </button>
                        <button 
                          ...
                          className={clsx(
                            "flex-1 py-1.5 rounded-md text-[10px] font-bold transition-all",
                            theme === "dark" ? "bg-slate-800 text-primary-400 shadow-sm border border-slate-700" : "text-slate-500"
                          )}
                        >
                          Dark
                        </button>
                ...
                <section>
                  <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Safeguards</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <Toggle 
            ...
            {activeTab === "security" && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <section>
                  <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Privacy Core</h3>
                  ...
                  <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                    <div className="flex flex-col sm:flex-row items-center gap-6 relative z-10">
                      <div className={clsx(
                        ...
                        pinHash ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"
                      )}>
                        <ShieldAlert size={32} />
                      </div>
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        <div className="flex items-center justify-center sm:justify-start gap-2 mb-1">
                          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">Security PIN Control</h4>
                          {pinHash && (
                            <div className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 text-[8px] font-black uppercase tracking-tighter border border-emerald-500/10">Active</div>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {activeTab === "booster" && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <section>
                  <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Engine Preferences</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <Toggle 
                ...
                <section>
                  <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Windows Hub</h3>
                  <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:bg-slate-50/50 dark:hover:bg-slate-900/50">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100">Native Disk Cleanup</h4>
                        <p className="text-[10px] text-slate-500">Coordinate Booster targets with Windows core cleanmgr.</p>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[11px] font-bold text-slate-600 dark:text-slate-300">Configure Native Flags</span>
                      </div>
