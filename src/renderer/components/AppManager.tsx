...
const AppManager: React.FC = () => {
  ...
  const handleUninstall = async (app: AppInfo) => {
    if (app.isUWP) {
      if (window.confirm(`Uninstall Windows App: ${app.name}?`)) {
        ...
        const res = await (window as any).electronAPI.uninstallUWPApp(
          app.fullName,
        );
        if (res.success) {
          fetchData(true);
    ...
    if (!app.uninstallString) {
      if (
        window.confirm(
          `No uninstaller found for ${app.name}. Use Force Uninstall (Powerful Scan)?`,
        )
      ) {
    ...
    if (window.confirm(`Uninstall ${app.name}?`)) {
      setUninstalling(app.name);
      const res = await (window as any).electronAPI.runUninstaller(
        app.uninstallString,
      );

      if (res.success) {
        setScanning(true);
        const scanRes = await (window as any).electronAPI.findAppLeftovers(
          app.name,
          app.installLocation,
        );
        ...
        alert(
          `Uninstall Failed. Error: ${res.error}\n\nRunning Force Uninstall scan...`,
        );
        handleForceUninstall(app);
      }
    }
  ...
  const handleForceUninstall = async (app: AppInfo) => {
    setUninstalling(app.name);
    setScanning(true);
    const scanRes = await (window as any).electronAPI.forceUninstall(
      app.name,
      app.installLocation,
    );
    setLeftovers(scanRes);
  ...
  const filteredApps = (
    currentView === "desktop" ? installedApps : uwpApps
  ).filter(
    (a) =>
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden relative">
          ...
          >
            ...
            >
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center gap-4 bg-emerald-500/5">
                ...
                <div>
                  <h2 className="text-2xl font-black text-slate-800 dark:text-white">
                    Powerful Scan Result
                  </h2>
                  <p className="text-slate-500 font-medium">
                    Found remnants left behind by {uninstalling}
                  </p>
                </div>
              </div>
              ...
              <div className="p-8 max-h-[400px] overflow-y-auto scrollbar-thin space-y-6">
                <div>
                  ...
                  <div className="space-y-2">
                    {leftovers.files.map((p, i) => (
                      <div
                        key={i}
                        className="text-[10px] font-mono p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-800 truncate"
                      >
                        {p}
                      </div>
                    ))}
                ...
                <div>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Zap size={14} /> Registry Entries (
                    {leftovers.registry.length})
                  </h3>
                  <div className="space-y-2">
                    {leftovers.registry.map((r, i) => (
                      <div
                        key={i}
                        className="text-[10px] font-mono p-3 bg-rose-500/5 rounded-xl text-rose-500/80 border border-rose-500/10 truncate"
                      >
                        {r}
                      </div>
                    ))}
                    {leftovers.registry.length === 0 && (
                      <p className="text-sm italic text-slate-400">
                        No leftover registry keys found.
                      </p>
                    )}
                  </div>
                </div>
      ...
      {/* Header */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-4 z-10">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-primary-500/50 transition-all shadow-sm"
              title="Refresh"
            >
              <RefreshCw
                size={16}
                className={clsx(loading && "animate-spin", "text-slate-500")}
              />
            </button>
          </div>
        </div>
      ...
      {/* Main Area */}
      <div className="flex-1 overflow-auto p-4 md:p-8 scrollbar-thin relative z-0">
        {error ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-12 bg-rose-500/5 rounded-[40px] border border-rose-500/10 mb-8">
            ...
            <button
              onClick={() => fetchData(true)}
              className="px-8 py-3 bg-primary-500 text-white rounded-2xl font-bold shadow-lg shadow-primary-500/20"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="h-full flex flex-col items-center justify-center p-8">
            {orphans.length === 0 ? (
              <div className="max-w-md text-center bg-white dark:bg-slate-900 p-12 rounded-[40px] border border-dashed border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/20">
                <Ghost
                  size={80}
                  className="text-slate-300 dark:text-slate-700 mx-auto mb-6"
                />
                <h2 className="text-2xl font-black mb-2 dark:text-white">
                  Residual Cleanup
                </h2>
                <p className="text-slate-500 mb-8 font-medium">
                  Scan for folders and files left behind by previously
              ...
              <div className="w-full h-full flex flex-col">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-xl font-black dark:text-white">
                    Found {orphans.length} Orphan Traces
                  </h2>
                  <button
                    onClick={handleOrphanScan}
                    className="flex items-center gap-2 text-primary-500 font-bold hover:underline"
                  >
                ...
                {filteredApps.map((app) => (
                  ...
                  >
                    ...
                    <div className="w-[320px] min-w-0 pr-4">
                      ...
                      {app.isUWP && (
                        <span className="text-[9px] font-black text-sky-500 uppercase tracking-tighter bg-sky-500/5 px-1.5 rounded">
                          Windows App
                        </span>
                      )}
                    </div>
