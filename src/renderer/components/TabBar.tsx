...
const TabBar: React.FC<TabBarProps> = ({ side }) => {
  const tabs = useStore((state) => side === 'left' ? state.leftTabs : state.rightTabs)
  const activeTabId = useStore((state) => side === 'left' ? state.activeLeftTabId : state.activeRightTabId)
