"use strict";
const main = require("./main-mW6E5sAs.cjs");
...
const { MAX_LENGTH: MAX_LENGTH$1, MAX_SAFE_INTEGER } = constants$2;
...
const parse$2 = (version2, options, throwErrors = false) => {
...
const readFileSync$1 = (path2) => {
  ...
  const bytesRead = fs$3.readSync(fd, buffer2, 0, MAX_LENGTH, 0);
  fs$3.close(fd, () => {
  });
  return buffer2.subarray(0, bytesRead);
};
const readFile$1 = (path2) => new Promise((resolve, reject) => {
  fs$3.open(path2, "r", (err, fd) => {
    if (err) {
      reject(err);
    } else {
      const buffer2 = Buffer.alloc(MAX_LENGTH);
      fs$3.read(fd, buffer2, 0, MAX_LENGTH, 0, (_, bytesRead) => {
...
const safeCommand = () => {
  if (!commandOut) {
    return new Promise((resolve) => {
      childProcess$1.exec(command, (err, out) => {
        commandOut = err ? " " : out;
        resolve(commandOut);
      });
    });
...
const safeCommandSync = () => {
  if (!commandOut) {
    try {
      commandOut = childProcess$1.execSync(command, { encoding: "utf8" });
    } catch (_err) {
      commandOut = " ";
    }
  }
...
const debuglog = util$4.debuglog("sharp");
...
function grayscale$1(grayscale) {
